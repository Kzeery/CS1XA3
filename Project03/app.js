const mainSocketListener = require("./exports/mainsocket"), // Websocket listener for the index pages
      routes             = require("./exports/routes.js"), // Routes
...
app.use("/e/kzeery", express.static(__dirname + "/public"));
...
const server  = http.Server(app),
      io      = socketIO(server, {path: "/e/kzeery/socket.io"}),
      urls    = [];
...
// Using the routes defined in exports/routes.js
app.use("/e/kzeery", routes);
...
app.get("/e/kzeery/game/:id", function(req, res) {
    if(urls.includes("/game/" + req.params.id)) {
        return res.render("game");
    }
    req.flash("error", "No game found with that id!");
